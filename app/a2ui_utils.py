...
def a2ui_callback(
    ...
    for part in llm_response.content.parts:
        ...
        # Extract readable text from A2UI Text components for text evaluation and CLI harnesses
        summary_lines = []
        for m in messages:
            if "surfaceUpdate" in m:
                for comp_wrapper in m["surfaceUpdate"].get("components", []):
                    comp = comp_wrapper.get("component", {})
                    if "Text" in comp:
                        txt_val = comp["Text"].get("text", {}).get("literalString")
                        if txt_val:
                            summary_lines.append(txt_val)
        summary_text = "\n".join(summary_lines).strip()
        if not summary_text:
            summary_text = "Structured event operations interface updated."

        new_parts = [types.Part(text=summary_text)] + [_wrap_a2ui_part(m) for m in messages]
        return LlmResponse(
